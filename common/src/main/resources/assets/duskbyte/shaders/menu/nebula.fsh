...
// Adapted from Processing Shader Examples by genekogan
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float n = surface3(vec3(position * sin(time * 0.1), time * 0.05)
        * mat3(1, 0, 0, 0, 0.8, 0.6, 0, -0.6, 0.8), 0.9);
    float n2 = surface3(vec3(position * cos(time * 0.1), time * 0.04)
        * mat3(1, 0, 0, 0, 0.8, 0.6, 0, -0.6, 0.8), 0.8);
    float lum = length(n);
    float lum2 = length(n2);

    vec3 tc = pow(vec3(1.0 - lum),
        vec3(sin(position.x) + cos(time) + 4.0,
             8.0 + sin(time) + 4.0,
             80.0));
    vec3 tc2 = pow(vec3(1.1 - lum2),
        vec3(5.0,
             position.y + cos(time) + 7.0,
             sin(position.x) + sin(time) + 2.0));
    vec3 curr_color = (tc * 6.8) + (tc2 * 2.5);

    // Stars
    float scale = sin(0.9 * time) + 3.0;
    vec2 position2 = (((fragCoord.xy / iResolution.xy) - 0.5) * scale);
    float gradient = 0.0;
