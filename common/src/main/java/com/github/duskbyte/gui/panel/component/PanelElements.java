package com.github.duskbyte.gui.panel.component;

import com.github.duskbyte.graphics.renderers.RectRenderer;
import com.github.duskbyte.graphics.renderers.RoundRectOutlineRenderer;
...
public class PanelElements {
    ...
    public static void drawSwitch(RoundRectRenderer roundRectRenderer, RectRenderer rectRenderer, PanelLayout.Rect rect, float toggleProgress, float hoverProgress) {
        drawSwitch(roundRectRenderer, rectRenderer, null, rect, toggleProgress, hoverProgress);
    }

    public static void drawSwitch(RoundRectRenderer roundRectRenderer, RectRenderer rectRenderer, RoundRectOutlineRenderer roundRectOutlineRenderer, PanelLayout.Rect rect, float toggleProgress, float hoverProgress) {
        ...
        rectRenderer.addRect(rect.x(), rect.y(), rect.width(), rect.height(), track);
        if (outline.getAlpha() > 0) {
            rectRenderer.addRect(rect.x(), rect.y(), rect.width(), rect.height(), outline);
            rectRenderer.addRect(rect.x() + 1, rect.y() + 1, rect.width() - 2, rect.height() - 2, track);
        }
        if (hoverProgress > 0.02f) {
            ...
            rectRenderer.addRect(haloX, haloY, haloSize, haloSize,
                    MD3Theme.stateLayer(MD3Theme.TEXT_PRIMARY, hoverProgress, 18));
        }
        rectRenderer.addRect(knobX, knobY, knobSize, knobSize, knob);
    }
