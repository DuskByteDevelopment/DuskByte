...
import com.github.duskbyte.gui.panel.component.setting.KeybindSettingRow;
...
import com.github.duskbyte.settings.impl.KeybindSetting;
...
public class AddonClientSettingTab implements ClientSettingTabView {
    ...
    private static final TranslateComponent emptyComponent = DuskTranslate.create("gui", "addon.empty");
    private static final TranslateComponent noSettingsComponent = DuskTranslate.create("gui", "addon.no_settings");
    private static final TranslateComponent idComponent = DuskTranslate.create("gui", "addon.info.id");
    private static final TranslateComponent versionComponent = DuskTranslate.create("gui", "addon.info.version");
    private static final TranslateComponent authorsComponent = DuskTranslate.create("gui", "addon.info.authors");
    private static final TranslateComponent modulesComponent = DuskTranslate.create("gui", "addon.info.modules");
    ...
    public void render(GuiGraphicsExtractor guiGraphics, PanelLayout.Rect bounds, int mouseX, int mouseY, float partialTick) {
        ...
        List<DuskAddon> addons = AddonManager.INSTANCE.getAddons();
        DuskAddon selectedAddon = resolveSelectedAddon(addons);
        ...
        if (rebuildContent) {
            ...
            rowEntries.clear();
            List<String> addonIds = addons.stream().map(DuskAddon::getAddonId).toList();
    ...
    private DuskAddon resolveSelectedAddon(List<DuskAddon> addons) {
        if (addons.isEmpty()) {
        ...
        DuskAddon fallback = addons.getFirst();
    ...
    private void buildAddonListRow(PanelUiTree.Scope scope, DuskAddon addon, PanelLayout.Rect rowBounds, float hoverProgress, float selectedProgress) {
    ...
    private void buildAddonInfo(PanelUiTree.Scope scope, DuskAddon addon, PanelLayout.Rect infoBounds) {
    ...
    private PanelLayout.Rect getDetailInfoBounds(PanelLayout.Rect detailPanelBounds, DuskAddon addon) {
        return new PanelLayout.Rect(
    ...
    private PanelLayout.Rect getDetailSettingsViewport(PanelLayout.Rect detailPanelBounds, DuskAddon addon) {
    ...
    private float getDetailInfoHeight(PanelLayout.Rect detailPanelBounds, DuskAddon addon) {
    ...
    private boolean shouldRebuild(PanelLayout.Rect bounds, int mouseX, int mouseY, List<DuskAddon> addons, DuskAddon selectedAddon, List<Setting<?>> selectedSettings, int guiHeight, long contentSignature) {
        ...
        List<String> addonKeys = addons.stream().map(DuskAddon::getAddonId).toList();
    ...
    private void rememberSnapshot(PanelLayout.Rect bounds, int mouseX, int mouseY, List<DuskAddon> addons, DuskAddon selectedAddon, List<Setting<?>> selectedSettings, int guiHeight, long contentSignature) {
        ...
        lastAddonKeys = addons.stream().map(DuskAddon::getAddonId).toList();
        lastVisibleSettings = selectedSettings.stream().map(Setting::getName).toList();
        lastContentSignature = contentSignature;
    }

    private long buildContentSignature(List<DuskAddon> addons, DuskAddon selectedAddon, List<Setting<?>> selectedSettings) {
        long signature = 17L;
