...
import com.github.duskbyte.assets.config.LegacyConfigMigrator;
...
public class ConfigManager {
    ...
    private static final Path configDir = Paths.get(System.getProperty("user.home"), ".duskbyte");
    ...
    public void initConfig() {
        try {
            ...
            DuskByte.LOGGER.error("初始化配置失败", e);
        }
    }
    ...
    public synchronized List<String> listConfigs() {
        try {
            ...
            DuskByte.LOGGER.error("列出配置失败", e);
    ...
    public synchronized void reload() {
        try {
            ...
            DuskByte.LOGGER.error("重载配置失败", e);
        }
    }
    ...
    public synchronized void saveNow() {
        try {
            ...
            DuskByte.LOGGER.error("保存配置失败", e);
        }
    }
    ...
    private void applyModuleFromDisk(Module module, Path configStorageDir) {
        ...
        try {
            ...
            DuskByte.LOGGER.error("读取模块配置失败: {}", file, e);
        }
    }
    ...
    private void saveModuleToDisk(Module module, Path configStorageDir) throws IOException {
        ...
        try {
            ...
            DuskByte.LOGGER.error("写入模块配置失败: {}", file, e);
            throw e;
        }
    }
    ...
    private Path getAddonSettingsFile(Path configStorageDir, DuskAddon addon) {
        return configStorageDir.resolve(addon.getAddonId()).resolve(ADDON_SETTINGS_FILE_NAME);
    }

    private void applyAddonFromDisk(DuskAddon addon, Path configStorageDir) {
        ...
        try {
            ...
            DuskByte.LOGGER.error("读取 addon 配置失败: {}", file, e);
        }
    }

    private void saveAddonToDisk(DuskAddon addon, Path configStorageDir) throws IOException {
        if (addon == null || addon.getSettings().isEmpty()) {
            return;
        }
        ...
        try {
            ...
            DuskByte.LOGGER.error("写入 addon 配置失败: {}", file, e);
            throw e;
        }
    }

    private JsonObject buildAddonObject(DuskAddon addon) {
        JsonObject obj = new JsonObject();
        obj.addProperty("version", CONFIG_VERSION);
    ...
    private synchronized void saveFriends(Path configStorageDir) throws IOException {
        ...
        try {
            ...
            DuskByte.LOGGER.error("写入好友文件失败: {}", friendFile, e);
            throw e;
        }
    }
    ...
    private synchronized void loadFriends(Path configStorageDir) {
        ...
        try {
            ...
            DuskByte.LOGGER.error("读取好友文件失败: {}", friendFile, e);
        }
    }
    ...
    private void resetAddonsToDefaults(List<DuskAddon> addons) {
        if (addons == null) {
            return;
        }
    ...
    private void applyToAddons(List<DuskAddon> addons) {
        if (addons == null) {
            return;
        }
        for (DuskAddon addon : addons) {
    ...
    private void saveAddonsToDisk(List<DuskAddon> addons, Path configStorageDir) throws IOException {
        if (addons == null) {
            return;
        }
        for (DuskAddon addon : addons) {
