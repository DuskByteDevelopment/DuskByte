package com.github.duskbyte.utils.render;
...
public class DuskGui implements AutoCloseable {
    ...
    private final List<DuskGui.Draw> draws = new ArrayList<>();
    private final List<DuskGui.MeshToDraw> meshesToDraw = new ArrayList<>();
    ...
    private void draw(GpuBufferSlice fogBuffer) {
        if (!this.draws.isEmpty()) {
            ...
            for (DuskGui.Draw draw : this.draws) {
    ...
    private void executeDrawRange(
        ...
        try (RenderPass renderPass = RenderSystem.getDevice()
            ...
            for (int i = startIndex; i < endIndex; i++) {
                DuskGui.Draw draw = this.draws.get(i);
    ...
    private void prepareText() {
        this.renderState.forEachText(text -> {
            ...
            text.ensurePrepared().visit(new Font.GlyphVisitor() {
                {
                    Objects.requireNonNull(DuskGui.this);
                }
                ...
                private void accept(TextRenderable glyph) {
                    DuskGui.this.renderState.addGlyphToCurrentLayer(new GlyphRenderState(pose, glyph, scissor));
                }
            });
        });
    }
    ...
    private void recordMesh(BufferBuilder bufferBuilder, RenderPipeline pipeline, TextureSetup textureSetup, @Nullable ScreenRectangle scissorArea) {
        ...
        if (mesh != null) {
            this.meshesToDraw.add(new DuskGui.MeshToDraw(mesh, pipeline, textureSetup, scissorArea));
        }
    }
    ...
    private void recordDraws() {
        ...
        for (DuskGui.MeshToDraw meshToDraw : this.meshesToDraw) {
    ...
    private void executeDraw(DuskGui.Draw draw, RenderPass renderPass, GpuBuffer indexBuffer, VertexFormat.IndexType indexType) {
