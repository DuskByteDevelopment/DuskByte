...
import com.github.duskbyte.utils.render.ColorUtils;
...
public class CaptureMarkESP {
    ...
    private static final RenderPipeline TARGET_ICON_PIPELINE = RenderPipeline.builder(RenderPipelines.GUI_TEXTURED_SNIPPET)
            .withLocation("pipeline/duskbyte_target_icon")
            .withColorTargetState(new ColorTargetState(BlendFunction.TRANSLUCENT))
            .withDepthStencilState(new DepthStencilState(CompareOp.ALWAYS_PASS, false))
            .withCull(false)
            .build();

    private static final RenderType TARGET_ICON_LAYER = RenderType.create("duskbyte_target_icon", RenderSetup.builder(TARGET_ICON_PIPELINE)
